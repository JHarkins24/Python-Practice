...
class DepthSearch:
    ...
    def transform_word(self,start_word,goal_word,limit_level):
        ...
        while(len(self.fringe) != 0):
            ...
            if(self.check_goal(current_node)):
               ...
               solution_found = True
               print("\n\nPath :",self.findNode(current_node.data, self.vertices_links).ancestry, "Level: ",self.findNode(current_node.data, self.vertices_links).level)
               break;
            else:
                if(current_node.level >= limit_level):
                    solution_found = False  
    ...
    def resetData(self):
        self.dictionary = ["try", "tim" , "tom" , "com", "toy" ,"cop","cuz","cob", "cup", "coy", "fry", "cry", "bay",
                  "lay", "boy", "bow","cow", "fey", "key","lop"]
        self.fringe = []  
        self.vertices_links = []
        self.visited_nodes = []
        ...
        for i in range(len(current_word.data)):
            for char in self.alphabet:
                ...
                if next_word in self.dictionary:
                    ...
                    new_node.level = current_word.level+1
...
myBFS = DepthSearch()
level = 0
while  myBFS.transform_word(start_word, goal_word,level) == False:
